...
RUN adduser --disabled-password --home ${DJANGO_ROOT} ${DJANGO_USER} && \
    mkdir -p ${DJANGO_ROOT} ${DJANGO_ROOT}/static ${DJANGO_ROOT}/media ${DJANGO_ROOT}/logs && \
    chown -R ${DJANGO_USER} ${DJANGO_ROOT} ${DJANGO_LOGDIR} ${DJANGO_ROOT}/static ${DJANGO_ROOT}/media
...
VOLUME ["${DJANGO_ROOT}", "${DJANGO_LOGDIR}", "${DJANGO_ROOT}/media", "${DJANGO_ROOT}/static"]
